...
1. Add an entry in `/etc/hosts` file
    ...
    k get secret cockroachdb-node -n cockroach-operator-system -o jsonpath='{.data.ca\.crt}' | base64 -d > ca.crt
    k get secret cockroachdb-root -n cockroach-operator-system -o jsonpath='{.data.tls\.crt}' | base64 -d > client.root.crt
    k get secret cockroachdb-root -n cockroach-operator-system -o jsonpath='{.data.tls\.key}' | base64 -d > client.root.key

    ~/gitrepo/sdutta9/CRDB/cockroachdb (main*) » cockroach sql --certs-dir=./certs --host=cockroachdb.example.com
    #
    # Welcome to the CockroachDB SQL shell.
    # All statements must be terminated by a semicolon.
    # To exit, type: \q.
    ...
    failed to connect to `host=cockroachdb.example.com user=root database=`: tls error (EOF)
    Failed running "sql"

    sudo tcpdump -v -nnni any host cockroachdb.example.com -w test.pcap
